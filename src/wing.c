...
Wing *Wing_Construct(int naca_m,
    ...
    wing->b_wing_on_wing = AllocateDoubles(num_panels * num_panels);
    wing->surface_areas = AllocateDoubles(num_panels);
    wing->right_hand_side = AllocateDoubles(num_panels);
    wing->wake_vortex_strengths = AllocateDoubles(max_num_wake_rings);
    wing->bound_vortex_strengths = AllocateDoubles(num_panels);
    wing->previous_bound_vortex_strengths = AllocateDoubles(num_panels);
    ...
    wing->normal_vectors = Vector3D_Allocate(num_panels);
    wing->surface_points = Vector3D_Allocate(num_points);
    wing->control_points = Vector3D_Allocate(num_panels);
    ...
    wing->chordwise_tangent_vectors = Vector3D_Allocate(num_panels);
    wing->horizontal_velocity_buffer = Vector3D_Allocate(wing->num_spanwise_panels);

    Wing_ComputeSurfacePoints(wing);
    Wing_ComputeSurfaceVectors(wing);
    Wing_ComputeSurfaceAreas(wing);
    Wing_ComputeControlPoints(wing);
    Wing_ComputeBoundRingPoints(wing);
...
void Wing_GetCorners(const Wing *wing, Geometry geometry, int i, int j, Vector3D **corners) {
    ...
    Vector3D *points = Wing_GetPoints(wing, geometry, &num_rows, &num_cols);
    ...
    if (indices[2] > num_points - 1) {
        fprintf(stderr, "Wing_GetCorners: index %zu is out of bounds for array of size %zu", indices[2], num_points);
    }
...
void Wing_ComputeSurfaceVectors(Wing *wing) {
    ...
    for (int i = 0; i < wing->num_chordwise_panels; i++) {
        for (int j = 0; j < wing->num_spanwise_panels; j++) {
            ...
            Wing_GetCorners(wing, SURFACE_POINTS, i, j, corners);
